...
#                          abs(math.ceil(diff_abs.max())))
    ...
    ax.set_title(f'{na}_$c_\mathrm{{{var}}}$$_\mathrm{{,ECHAM}} - c_\mathrm{{{var}}}$$_\mathrm{{,ATom}}$ in $\mu g m^{-3}$')
    ...
    plt.savefig(f'{global_vars.plot_dir}{na}_diff_abs.pdf', bbox_inches='tight')
...
def scatter_plot(c_echam_txy,c_atom,statistical_quantities,exp):
    ...
    for i in range(len(statistical_quantities)):
        plt.gcf().text(0.11, -(i+3)*0.04, f'{statistical_quantities[i][0]} = {statistical_quantities[i][1]:.2f} {statistical_quantities[i][2]}')
...
def plot_one_pannel(c_echam,c_atom,ds_atom_vs_daily):
    ...
    for idx, na in enumerate(list(c_echam.keys())):
        ...
        std_model, std_obs, RMSE, mean_bias, normalized_mean_bias, pearsons_coeff, statistical_quantities = statistics.get_statistics(
            c_echam_txy, c_atom)
        scatter_plot(c_atom, c_atom, statistical_quantities, na)

        fig, ax = plt.subplots(1, 1, figsize=(8, 6))
        fig_na = f'Daily concentration {units}'
        plot_scatter_improve(ax, c_atom, c_echam_txy, fig_na, std_obs)
...
def plot_multipannel(reg_data):
    ...
    for idx, ex in enumerate(reg_data.keys()):
        ...
        for i, na in enumerate(reg_data[ex].keys()):
            ...
            c_echam_txy = ds_atom_vs_daily['echam_data']
            c_atom = ds_atom_vs_daily['atom_data']

            std_model, std_obs, RMSE, mean_bias, normalized_mean_bias, pearsons_coeff, _ = statistics.get_statistics(c_atom, c_echam_txy)
            stat = f'(RMSE: {RMSE:.2f}, bias:{normalized_mean_bias:.2f}, R: {pearsons_coeff:.2f})'
        ...
        axes[4].set_ylabel(f'Model {mo_var_title}', fontsize='16')

        axes[4].set_xlabel(f'ATom {at_var}', fontsize='16')
        axes[5].set_xlabel(f'ATom {at_var}', fontsize='16')
        axes[6].set_xlabel(f'ATom {at_var}', fontsize='16')
