...
def plot_flight_heights():
    ...
    for i in range(4):
        ...
        dates.append([datetime.strptime(str(m),
                                        '%Y-%m-%dT%H:%M:%S.000000000') for m in ds_atom_time['time'].values])
    ...
    ax = subfig1.subplots(nrows=1,
                          ncols=1)
    for i,axs in enumerate(colors):
        ax.scatter(oa_mass[i],
                   altitude[i],
                   c = colors[i],
                   label=names[i])
    ...
    ax.set_ylabel('Flight Height (km)',
                  fontsize = 12)
...
def plot_regions_map(reg_data):
    ...
    :param reg_data: dictionary with region definition and data
    :return: None
    """
    ...
    for i, na in enumerate(reg_data.keys()):
        cb = ax.scatter(reg_data[na]['longitud'],
                        reg_data[na]['latitud'],
                        c=color_reg[i],
                        label=na,
    ...
    plt.savefig(f'{global_vars.plot_dir}atom_region_sel.png',
                dpi=300)
